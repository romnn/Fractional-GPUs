...
#error                                                                         \
    "FGPU_TEST_MEM_COLORING_ENABLED not defined. Needed for reverse engineering"
#endif
...
success:
  /* Just checks */
  assert(is_cacheline_evicted(base, upper_bound, threshold, &reached_end, &addr,
                              &local_sum) == true);
  assert(is_cacheline_evicted(base, lower_bound, threshold, &reached_end, &addr,
                              &local_sum) == false);
  /* Success */
  *psum = sum;
